<?php
/**
 * Privacy consent compliance plugin for forms that collect personal data
 *
 * @package     Joomla.Plugin
 * @subpackage  Fabrik.form.consent
	...
	public function onAfterProcess()
	{
		...
		if ($formModel->isNewRecord())
		{
			...
			$values  = array(
				...
				$listId,
				$db->quote($contactId),
				$db->quote($acymailingUserId),
				$db->quote($acymailingListIds),
				$db->quote($contactMessage),
				$db->quote($acymailingMessage),
				$db->quote($_SERVER['REMOTE_ADDR'])
			);
			$query->insert($db->quoteName('#__fabrik_consent'))
